...
def update_resource_permit(profile: str ,class_name: str, racf_id: str, base: BasePermitTraits) -> int:
    ...
    if racfu_enabled:
        ...
        result = racfu({"operation": "alter", "admin_type": "permission", "resource": profile.upper(), "class": class_name.upper(), "auth_id": racf_id.upper(), "traits":  traits})
        return result.result["return_codes"]["racf_return_code"] == 0
    else:
        return 8
...
def delete_resource_permit(profile: str, class_name: str, racf_id: str) -> int:
    ...
    if racfu_enabled:
        result = racfu({"operation": "delete", "admin_type": "permission", "resource": profile.upper(), "class": class_name.upper(), "auth_id": racf_id.upper()})
        return result.result["return_codes"]["racf_return_code"] == 0
    else:
        return 8
